-- ==================== FONCTIONS ADMIN POUR SUPABASE ====================
-- Ces fonctions doivent être exécutées dans l'éditeur SQL de Supabase

-- Supprimer d'abord toutes les fonctions existantes pour éviter les conflits
...
-- 1. Fonction pour supprimer un utilisateur définitivement
...
BEGIN
    ...
    -- Supprimer l'utilisateur de auth.users (cela supprimera automatiquement les sessions)
...
-- 2. Fonction pour modifier les informations d'un utilisateur
CREATE OR REPLACE FUNCTION update_user_info_admin(
    target_user_id UUID,
...
-- 3. Table pour les logs d'administration (si elle n'existe pas déjà)
...
-- 4. Fonction pour obtenir tous les utilisateurs avec leurs informations complètes
-- Supprimer d'abord la fonction existante si elle a un type de retour différent
...
-- 5. Fonction pour mettre à jour le statut d'un utilisateur
...
-- 6. Vue pour les statistiques admin
CREATE OR REPLACE VIEW admin_stats AS
SELECT 
    COUNT(*) as total_users,
    COUNT(CASE WHEN up.account_status = 'active' THEN 1 END) as active_users,
    COUNT(CASE WHEN up.account_status = 'pending' THEN 1 END) as pending_users,
    COUNT(CASE WHEN up.account_status = 'suspended' THEN 1 END) as suspended_users,
    COUNT(CASE WHEN up.account_status = 'expired' THEN 1 END) as expired_users
FROM auth.users u
LEFT JOIN public.user_profiles up ON u.id = up.user_id
WHERE u.email IS NOT NULL;

-- 7. Politique de sécurité pour les fonctions admin (optionnel)
-- Ces fonctions ne devraient être accessibles qu'aux administrateurs
-- Vous pouvez ajouter des vérifications de rôle si nécessaire

-- Exemple de vérification admin (à adapter selon votre système)
/*
